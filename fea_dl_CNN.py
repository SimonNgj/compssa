...
def model_arch1(withOP=True):
    ...
    if withOP:
        out = Dense(3, kernel_regularizer=regularizers.l2(0.001), activation='softmax')(out)
...
#            ip = Input(shape=(sizeD_ts, sizeD_va), name='main_input')
#    
#            x = Conv1D(32, 5, padding='same', kernel_initializer='he_uniform')(ip)
#            x = BatchNormalization()(x)
#            x = Activation('relu')(x)
#            x = MaxPooling1D(2)(x)
#
...
#            plt.close()
            ...
            print(cr)
            print(cm)
            f = open(str(tim)+'_'+skill+'_'+str(win)+'_'+str(over)+'_report_CNN_fea.txt', 'a+')
            f.write("-------------------- Full NN -----------------------")
